...
describe 'Service specific award shows scraper' do
	...
	temp = csv_final.dup
	#temp.sub!(".csv", "")
    csv_output = temp + "#{run_date}master_csv.csv"

    CSV.open(csv_output, "w+") do |head|
    	head << ["Scraped Date","category","Series Name","Release_Year","Season Number","Episode Number","Episode Title","Links","Result","Error","Date Validated"]
    end
	...
	CSV.foreach(csv_input1) do |row|
		...
		if status == nil or !status.include?"PASS"
